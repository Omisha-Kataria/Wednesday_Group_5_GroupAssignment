...
//               SupplierDirectory suplierdirectory = business.getSupplierDirectory();
        ...
        // adding 50 suppliers
        int supplierCount = 50;
        for (int i = 0; i < supplierCount; i++) {
            supplierDirectory.newSupplier("Supplier " + (i+1));
        }
        
        // selecting any 30 suppliers to add 50 products to
        Random rand = new Random();
        
        
        
        
        Set<Supplier> pickedSuppliers = new HashSet<>();
        int productCount = 50;
        int pickedSupplierCount = 30;
        
        while (pickedSuppliers.size() < pickedSupplierCount) {
            Supplier supplier = supplierDirectory.pickRandomSupplier();
            if (pickedSuppliers.contains(supplier)) {
                continue;
            }
            pickedSuppliers.add(supplier);
            ...
            for (int i = 0; i < productCount; i++) {
                ...
                int range = 1000;
                int lowerPriceLimit = 5000;
                int upperPriceLimit = 10000;
                int randomFloor = rand.nextInt(lowerPriceLimit, lowerPriceLimit + range);
                int randomCeiling = rand.nextInt(upperPriceLimit - range, upperPriceLimit);
                int randomTarget = rand.nextInt(randomFloor, randomCeiling);
...
// Create Customers
        ...
        // adding 300 customers
        int customerCount = 300;
        for (int i = 0; i < customerCount; i++) {
            Person person = persondirectory.newPerson("Customer #" + (i+1));
...
// Process Orders on behalf of sales person and customer
        ...
        // adding the prepolated orders to prepopulated suppliers
        order1.setSupplier(supplier1);
        supplier1.addOrder(order1);
        order12.setSupplier(supplier1);
        supplier1.addOrder(order12);
        

        // adding 1-3 orders to every customer, each order 1-10 items
        int orderCount = rand.nextInt(1, 3);
        int orderItemCount = rand.nextInt(1, 10);
        ...
        for (int customerIndex = 0; customerIndex < customerCount; customerIndex++) {
            ...
            for (int orderIndex = 0; orderIndex < orderCount; orderIndex++) {
                ...
                Order order = masterorderlist.newOrder(randomCustomer, salespersonprofile);
                Product product = pc.pickRandomProduct();
                int actualprice = rand.nextInt(product.getFloorPrice(), product.getCeilingPrice());
                
                order.newOrderItem(product, actualprice, orderItemCount);
                order.setSupplier(randomSupplier);
